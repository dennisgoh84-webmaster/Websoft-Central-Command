#!/usr/bin/env bash
#
# Central Command -- host-side upgrade agent.
#
...
# scripts/install-upgrade-agent.sh, which deploy.sh and upgrade.sh both
...
#      Upgrade screen shows that as current / latest / "behind by N".
#   2. Takes the next pending row of `cc_upgrade_requests` (the CC Upgrade screen
#      inserts them), runs `scripts/upgrade.sh <target_ref>` with its
...
#   UPGRADE_AGENT_UPGRADE_CMD  command run instead of scripts/upgrade.sh
#
...
TOKEN="${CC_UPGRADE_AGENT_TOKEN:-}"
[ -n "$TOKEN" ] || { echo "CC_UPGRADE_AGENT_TOKEN is not set in .env -- run scripts/install-upgrade-agent.sh" >&2; exit 1; }
API="${UPGRADE_AGENT_API:-http://127.0.0.1:${CC_HTTP_PORT:-8080}/api/cc-upgrade/agent}"
UPGRADE_CMD="${UPGRADE_AGENT_UPGRADE_CMD:-$ROOT/scripts/upgrade.sh}"
...
[ "$(id -u)" -ne 0 ] || fail "refusing to run as root -- files it wrote would be root-owned and block $ROOT's owner. Run it as that user (sudo scripts/install-upgrade-agent.sh sets the timer up that way)."
