...
run by hand, backup and smoke test included. So the deploy that
brings in this version is the last one that needs SSH.
...
- No sudo when the installer ran? Run
  `sudo ./scripts/install-upgrade-agent.sh` once.
- **Client installs** are upgraded the same way from **Client
  ...
  supported yet" until it has been upgraded once by hand.
